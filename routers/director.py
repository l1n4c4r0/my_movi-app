...
def update_director(id:int,director:Director):
    ...
    result = DirectorService(db).get_for_id(id)
    if not result:
        return JSONResponse(content={"message":"director don't gound","status_code":404})
    DirectorService(db).update_director(director)
    return JSONResponse(content={"message":"director update successfully",'status_code':202}, status_code=200)

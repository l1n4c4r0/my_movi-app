...
def create_ratings(rating: Rating):
    db = Session()
...
def get_ratings_id(id: int):
    ...
    return JSONResponse(content=jsonable_encoder(rating), status_code=200)


@rating_router.put('/ratings/{id}', tags=['ratings'])
def update_ratings(id: int, data: Rating):
    db = Session()
    result = RatingService(db).get_for_id(id)
    if not result:
        return JSONResponse(content={"message": "rating don't found", "status_code": 202})
    return JSONResponse(content={"message": "rating update succesfully", "status_code": 201})

@rating_router.delete('/ratings/{id}', tags=['ratings'])
def delete_ratings(id: int, data: Rating):
    db = Session()
    result = RatingService(db).get_for_id(id)
    if not result:
        return JSONResponse(content={"message": "rating update succesfully", "status_code": 202})
